cc      subroutine funx (iflag,nprop,nparam,xc,fvecc,iw,liw,w,lw)
      ...
      double precision effmass, ecbm, ekpt
...
C==== for the ideal zincblend structure, psf = 0        
       ...
       ekpt=1.d0/effmass*(kpc(1)**2+kpc(2)**2+kpc(3)**2)
c         call pwk(kp,vr,vc,-1,ev,proj,iproj(is),zz_st(1,1,is))

        write(6,300) kp(1),kp(2),kp(3),
     &        (27.211396/2*ekpt+ecbm)
        write(11,300) kp(1),kp(2),kp(3),
     &        (27.211396/2*ekpt+ecbm)
300     format(3(f10.7,1x),(f10.5,1x))
         enddo
        close(17)
        close(11)
